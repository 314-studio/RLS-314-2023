...
public class CameraController2 : MonoBehaviour
{
    ...
    //Pan Settings
    [Header("Pan Settings")] public float panSpeed = 15.0f;
    ...
    //Zoom Settings
    [Header("Zoom Settings")] public float zoomSpeed = 1.0f;
    ...
    //Rotation Settings
    [Header("Rotation Settings")] public float rotationSpeed = 50.0f;
    ...
    [Header("General Speed Settings")] public bool smoothing = true;
    public float smoothingFactor = 0.1f;


    //Generic Settings
    [Header("Generic Settings")] public bool adaptToTerrainHeight = true;
    ...
    private void Pan()
    {
        ...
        if (MouseInput())
        {
            ...
            if (Input.GetMouseButton(1))
            {
                ...
                {
                    if ((mouseClicked.y < Input.mousePosition.y) &&
                        ((Input.mousePosition.y - mouseClicked.y) > mouseDeadZone))
                        whereTo += Vector3.forward * (Input.mousePosition.y - mouseClicked.y) / 150 *
                                   mousePanMultiplier;

                    if ((mouseClicked.x < Input.mousePosition.x) &&
                        ((Input.mousePosition.x - mouseClicked.x) > mouseDeadZone))
                        whereTo += Vector3.right * (Input.mousePosition.x - mouseClicked.x) / 150 * mousePanMultiplier;

                    if ((mouseClicked.y > Input.mousePosition.y) &&
                        ((mouseClicked.y - Input.mousePosition.y) > mouseDeadZone))
                        whereTo += Vector3.back * (mouseClicked.y - Input.mousePosition.y) / 150 * mousePanMultiplier;

                    if ((mouseClicked.x > Input.mousePosition.x) &&
                        ((mouseClicked.x - Input.mousePosition.x) > mouseDeadZone))
    ...
    private void Zoom()
    {
        ...
        if (useLimits)
        {
            if ((transform.position.y >= zoomMin && whereTo.y < 0) ||
                (transform.position.y <= zoomMax && whereTo.y > 0))
                SmoothIt(whereTo);
        }
        else
        {
    ...
    private void SmoothIt(Vector3 whereTo)
    {
        ...
        {
            RaycastHit hit;
            if (Physics.Raycast(_camera.transform.position, _camera.transform.TransformDirection(Vector3.forward) * 5,
                    out hit, 5, groundLayer))
            {
