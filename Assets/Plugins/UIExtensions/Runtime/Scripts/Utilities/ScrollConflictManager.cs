        ...
        private void InitialiseConflictManager()
        {
            ...
            if (scrollOtherHorizontally)
            {
                if (_myScrollRect.horizontal)
                    DebugTool.Error("You have added the SecondScrollRect to a scroll view that already has both directions selected");
                if (!ParentScrollRect.horizontal)
                    DebugTool.Error("The other scroll rect does not support scrolling horizontally");
            }
            else if (!ParentScrollRect.vertical)
            {
                DebugTool.Error("The other scroll rect does not support scrolling vertically");
            }
